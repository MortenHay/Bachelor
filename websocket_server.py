...
async def update_all_droop_constants(connected: dict):
    total_capacity_inv = 1.0 / update_total_system_capacity(connected)
    tasks = []
    for websocket, values in connected.items():
...
async def handle_client(websocket):
    ...
    await update_all_droop_constants(connected_clients)
